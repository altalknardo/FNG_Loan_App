...
export function KYCRegistration({ onRegistrationComplete }: Props) {
  ...
  const verifyBvnAndAccount = () => {
    ...
    setTimeout(() => {
      ...
      if (isFintech) {
        toast.success(`Phone number verified! Account name: ${bankAccountName}`);
      } else {
  ...
  const handleFileUpload = (field: keyof KYCFormData, event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const validateStep = (currentStep: number): boolean => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.paymentType) {
        ...
        if (!formData.cardBrand.trim()) newErrors.cardBrand = "Card brand is required";
      }
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem("kycSubmissions", JSON.stringify([...existingSubmissions, newSubmission]));

      // Save payment method separately for quick access during payments
      if (formData.paymentType) {
        const paymentMethods = JSON.parse(localStorage.getItem("paymentMethods") || "[]");
        
        if (formData.paymentType === "bank") {
          // Check if bank account doesn't already exist
          const exists = paymentMethods.find(
            (pm: any) => pm.type === "bank" && pm.last4 === formData.accountNumber.slice(-4)
          );
          ...
          if (!exists) {
            const bankPayment = {
              ...
              bvn: formData.bvn, // Link BVN to bank account
          ...
          if (!exists) {
            ...
            paymentMethods.push(cardPayment);
          }
        }
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="phone">
                  Phone Number *
                  {isFintech && <span className="text-xs text-purple-600 ml-2">(Used for account verification)</span>}
                </Label>
                <div className="relative">
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="bvn">
                  ...
                  {isFintech && <span className="text-xs text-gray-500">(Optional for fintech banks)</span>}
                </Label>
                <div className="relative">
            ...
            <Alert>
              ...
              <AlertDescription>
                All your information is encrypted and stored securely. We comply with data protection regulations.
              </AlertDescription>
            </Alert>
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-6">
                Add your preferred payment method for contributions and loan repayments.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <Building2 className={`h-6 w-6 ${formData.paymentType === "bank" ? "text-blue-600" : "text-gray-600"}`} />
                    <span className={formData.paymentType === "bank" ? "text-blue-600" : "text-gray-600"}>Bank Account</span>
                  </button>
                  ...
                  >
                    <CreditCard className={`h-6 w-6 ${formData.paymentType === "card" ? "text-blue-600" : "text-gray-600"}`} />
                    <span className={formData.paymentType === "card" ? "text-blue-600" : "text-gray-600"}>Debit Card</span>
                  </button>
                </div>
                {errors.paymentType && (
              ...
              {formData.paymentType === "bank" && (
                <div className="space-y-4">
                    ...
                    {isFintech && (
                      <Alert className="bg-purple-50 border-purple-200">
                        ...
                        <AlertDescription className="text-purple-800 text-sm">
                          For {formData.bankName}, we'll verify your account using your registered phone number instead of BVN
                        </AlertDescription>
                      </Alert>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        className={`${errors.accountName ? "border-red-500" : ""} ${accountVerified ? "border-green-500 bg-green-50" : ""}`}
                  ...
                  {!accountVerified && formData.bankName && formData.accountNumber && (
                    isFintech ? (
                      ...
                      formData.phone && formData.phone.length >= 11 && (
                        <Alert className="bg-purple-50 border-purple-200">
                          <Phone className="h-4 w-4 text-purple-600" />
                          <AlertDescription className="text-purple-800 text-sm">
                            Click verify to confirm your account using your registered phone number
                          </AlertDescription>
                        </Alert>
                      )
                    ) : (
                      ...
                      formData.bvn && formData.bvn.length === 11 && (
                        <Alert className="bg-blue-50 border-blue-200">
                          ...
                          <AlertDescription className="text-blue-800 text-sm">
                            Click verify to fetch your account name directly from your bank records
                          </AlertDescription>
                        </Alert>
                      )
                    )
                    ...
                    >
                      {isBvnVerifying ? (
                        <>
                          ...
                          {isFintech ? "Verifying with Phone..." : "Verifying with BVN..."}
                        </>
                      ) : accountVerified ? (
                        <>
                        ...
                        <>
                          {isFintech ? <Phone className="h-4 w-4 mr-2" /> : <Shield className="h-4 w-4 mr-2" />}
                          {isFintech ? "Verify Account with Phone" : "Verify Account with BVN"}
                        </>
                      )}
                    </Button>
              ...
              {formData.paymentType === "card" && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.cardBrand} onValueChange={(value) => handleInputChange("cardBrand", value)}>
                      <SelectTrigger className={errors.cardBrand ? "border-red-500" : ""}>
            ...
            <Alert>
              <Shield className="h-4 w-4" />
              <AlertDescription>
                Your payment information is encrypted and stored securely. This will be your default payment method.
              </AlertDescription>
            </Alert>
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-6">
                Upload clear photos of your documents. Accepted formats: JPG, PNG, PDF (max 5MB each)
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className={`border-2 border-dashed rounded-lg p-6 text-center ${errors.idDocument ? "border-red-500" : "border-gray-300"}`}>
                  {formData.idDocument ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm">
                          {(() => {
                            ...
                            return filesMap[formData.idDocument]?.name || "Document uploaded";
                          })()}
                        </span>
                      </div>
                      <Button
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500 mb-2">
                  Utility bill, bank statement, or government-issued document (not older than 3 months)
                </p>
                <div className={`border-2 border-dashed rounded-lg p-6 text-center ${errors.proofOfAddress ? "border-red-500" : "border-gray-300"}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm">
                          {(() => {
                            const filesMap = JSON.parse(localStorage.getItem("kycFileData") || "{}");
                            return filesMap[formData.proofOfAddress]?.name || "Document uploaded";
                          })()}
                        </span>
                      </div>
                      <Button
              ...
              <div className="space-y-2">
                ...
                <div className={`border-2 border-dashed rounded-lg p-6 text-center ${errors.selfie ? "border-red-500" : "border-gray-300"}`}>
                  {formData.selfie ? (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <CheckCircle2 className="h-5 w-5 text-green-600" />
                        <span className="text-sm">
                          {(() => {
                            const filesMap = JSON.parse(localStorage.getItem("kycFileData") || "{}");
                            return filesMap[formData.selfie]?.name || "Selfie uploaded";
                          })()}
                        </span>
                      </div>
                      <Button
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-indigo-50 py-4 sm:py-8 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-4 sm:mb-8">
          ...
          <p className="text-sm sm:text-base text-gray-600">Complete your verification to get started</p>
        </div>
        ...
        <Card className="p-4 sm:p-6 mb-4 sm:mb-6">
          <div className="space-y-3 sm:space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-xs sm:text-sm">Step {step} of {totalSteps}</span>
            ...
            <div className="flex justify-between text-[10px] sm:text-xs text-gray-600">
              ...
              <span className={step >= 5 ? "text-blue-600" : ""}>Documents</span>
            </div>
          </div>
        </Card>
